"""Manual test script for RemotePortCheckTool."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run remote port availability check")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Port list to check (override settings)"
    )
...
def main() -> None:
    ...
    print(
        f"Running remote_port_check as {server.get('username')} on {server.get('host')}"
    )
    ...
    executor.connect()
    try:
        tool = RemotePortCheckTool()
        config = settings.get("pre_install", {}).get("port_check", {})
        result = tool.run(
            executor=executor,
            config=config,
            ports=args.ports,
